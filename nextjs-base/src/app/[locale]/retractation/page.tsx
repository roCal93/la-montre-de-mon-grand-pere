...
type RetractationData = {
  title?: string
  content?: string
  lastUpdated?: string
}

type RetractationResponse = {
  data?: RetractationData | null
}
...
export async function generateMetadata({
  ...
  const response = await fetchAPI<RetractationResponse>('/retractation', {
    locale,
    next: { revalidate: 60 },
  })
...
export default async function RetractationPage({
  ...
  if ((!data?.title || !data?.content) && locale !== defaultLocale) {
    const fallbackResponse = await fetchAPI<RetractationResponse>(
      '/retractation',
      {
